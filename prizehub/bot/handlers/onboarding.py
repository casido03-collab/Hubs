...
async def process_interests(message: Message, state: FSMContext, session: AsyncSession):
    ...
    if message.text == "✅ Готово":
        if not interests:
            await message.answer("Выберите хотя бы один интерес.", reply_markup=interests_keyboard())
            return
        # Save onboarding
    ...
    if message.text in INTERESTS:
        if message.text in interests:
            interests.remove(message.text)
        else:
            interests.append(message.text)
        await state.update_data(interests=interests)
        selected = ", ".join(interests) if interests else "—"
        await message.answer(
